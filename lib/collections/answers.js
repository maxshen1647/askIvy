...
Meteor.methods({  
	...
	answerInsert: function(answerAttributes) {    
      ...
      verified: Roles.userIsInRole(user, 'ivy'), //whether user is a harvard student
  ...
  // method for upvoting an answer
	upvote: function(answerId, authorId, questionId) {
		check(this.userId, String);
    ...
    if (! affected)      
      ...
      createUpvoteNotification(authorId, questionId, Meteor.user().username);
  },
